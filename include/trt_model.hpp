...
public:
    ...
    // virtual bool preprocess_fcos3d_cpu()=0;
    virtual bool preprocess_gpu()      = 0;
    // virtual bool preprocess_fcos3d_gpu()=0;
    virtual bool postprocess_cpu()     = 0;
    // virtual bool postprocess_fcos3d_cpu()=0;
    virtual bool postprocess_gpu()     = 0;
    // virtual bool postprocess_fcos3d_gpu()=0;
...
public:
    ...
    int    m_workspaceSize;
    // float* m_bindings[2]; //yolo输入输出都只为一个数据
    float* m_bindings[8]; //fcos3d输入3个，输出5个
    // float* m_inputMemory[2];
    // float* m_outputMemory[2];

    float* m_inputMemory[6]; //包括host端与device端的3 + 3数据
    float* m_outputMemory[10]; //包括host端与device端的5 + 5数据

    nvinfer1::Dims m_inputDims; //input_img
    nvinfer1::Dims m_outputDims; //output_bboxes
    nvinfer1::Dims m_c2iDims;
    nvinfer1::Dims m_c2i_invDims;
    nvinfer1::Dims m_labelDims;
    nvinfer1::Dims m_scoreDims;
    nvinfer1::Dims m_dirDims;
    nvinfer1::Dims m_attrDims;
